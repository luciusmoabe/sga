...
export function sqlSeguranca({ transacao = true } = {}) {
  const tabelas = [...TABELAS_APP, 'schema_migrations', 'auth_identidades', 'auth_sessoes', 'auth_fluxos',
    'auth_contas', 'auth_sessoes_senha', 'auth_tentativas'];
  return `${transacao ? 'begin;\n' : ''}do $seguranca$
declare tabela text; papel text; colunas text; sequencia text;
begin
  foreach tabela in array array[${tabelas.map(t => `'${t}'`).join(', ')}] loop

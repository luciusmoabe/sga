...
- [x] `scripts/diagnostico-supabase.sql`: diagnóstico somente leitura para um banco Agilis já existente (versões aplicadas, perfis, dados que bloqueiam a migração, volume, RLS e permissões), sem nomes nem e-mails no resultado; testado em PostgreSQL 18.4 local em banco vazio, antigo com dados problemáticos e migrado. `HOMOLOGACAO.md` ganhou a etapa 3.1 para banco existente, com a tabela de interpretação.
